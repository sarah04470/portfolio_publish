...
function validateBinaryVersion(...command) {
  ...
  if (stdout !== "0.14.39-1") {
    throw new Error(`Expected ${JSON.stringify("0.14.39-1")} but got ${JSON.stringify(stdout)}`);
  }
}
function isYarn() {
...
function installUsingNPM(pkg, subpath, binPath) {
  ...
  const esbuildLibDir = path2.dirname(require.resolve("@netlify/esbuild"));
  const installDir = path2.join(esbuildLibDir, "npm-install");
  fs2.mkdirSync(installDir);
  try {
    fs2.writeFileSync(path2.join(installDir, "package.json"), "{}");
    child_process.execSync(`npm install --loglevel=error --prefer-offline --no-audit --progress=false ${pkg}@${"0.14.39-1"}`, { cwd: installDir, stdio: "pipe", env });
...
async function downloadDirectlyFromNPM(pkg, subpath, binPath) {
  const url = `https://registry.npmjs.org/${pkg}/-/${pkg.replace("@netlify/", "")}-${"0.14.39-1"}.tgz`;
